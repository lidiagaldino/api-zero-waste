...
class MateriaisController {
  public async index(_req: Request, res: Response) {
    ...
    if (materiais)
      return res.status(StatusCodes.OK).json({ message: materiais });
  ...
  public async getByCatador(req: Request, res: Response) {
    ...
    const materiais = await Materiais.getByCatador(Number(id));

    return materiais.length > 0
      ? res.status(StatusCodes.OK).json(materiais)
  ...
  public async delete(req: Request, res: Response) {
    ...
    const materiais = await Materiais.getByCatador(Number(id_catador));
    const id = Number(id_catador);

    if (materiais.length > 1) {
      const rs = await Materiais.deleteMateriaisCatador(
        id,
        Number(id_material)
      );

      return rs
        ? res.status(StatusCodes.NO_CONTENT).json(rs)
        : res
            .status(StatusCodes.INTERNAL_SERVER_ERROR)
            .json({ message: "Algo deu errado" });
    } else {
      return res
        ...
        .json({ message: "Catador deve ter no minimo um material cadastrado" });
    }
  }
  ...
  public async getNoMateriaisCatador(req: Request, res: Response) {
    ...
    const materiais = await Materiais.noMaterial(Number(id_catador));
